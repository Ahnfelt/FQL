...
In section~\ref{sec:semantics} we give a operation small-step semantic for FQL and address some problems regarding the semantic for sets. After this we present our type system and show how this can be formalised in a way suitable for type inference with algorithm W. Section~\ref{sec:proofs} contains a some what non exhaustive collection of proofs necessary to show desirable properties of our type system. 

We will argue that a principle type exists, is unique and is found by out inference algorithm, but we will not cover the correctness of this algorithm in general. As a little proof of simplicity we have implemented the type inference system in Haskell which may be found in Appendix~\ref{app:inferenceCode}.

\section{Syntax}
\label{sec:syntax}

It was essential that a proposed language had a form of set comprehension allowing the programmer to join, constraint and filter elements from differen sets in a convinient way. We have choosen to use the form suggested in the paper with some minor modifications. However instead of incorporationg this feature directly into our language we have choosen to achive this by defining a syntactic sugar shell which we name \buzz{concrete syntax}. We will soon show how well-formed concrete syntax can be converted into the the syntax of the core language which we call the \buzz{abstract syntax}. 

The advantage of this approach is that our core language are smaller and thereby easyer to formalize, proof, and implement. A disadvantage is that the behaviour and correctness of the extended language now lies in the conversion function which may be hard to analyse. Anothe more practical implication could be that concise error reporting would be hard to implement.

\subsection{Abstract syntex}
\label{abstractSyntax}

A program in FQL is a term $t$ which is defined by the following gramma.
...
tuples. It would however be possible to achive tuples as iterated
pairs implementet as syntactic sugar. The lambda abstraction uses a pattern $p$ for binding variable and deconstruct pairs. Patterns are defined by
...
Certain terms is defined to be in \buzz{cannonical form} which means
...
rule. This will later serve to formulate progress as \buzz{ a term is
  either in cannonical form or can step to some other term}.  

Cannonical terms $c$ are defined by
...
The cannonical form for sets are non part of our abstract syntax. This
...
The pattern matching in the lambda apstraction bind variables to
...
\newpage
\begin{appendices}
\section{Type inference in haskell}
\label{app:inferenceCode}
