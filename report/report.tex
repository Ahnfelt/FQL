...
We define rules S-SetPickS and S-SetPickN to pick a member from a canonical set. As the sets are defined as mathematical sets, we do not provide any guarantees as to where the member is located in the set and again, the termination property is preserved by rule S-SetC. To provide a fail-safe semantic for the pick rules, we define the rule by using an \emph{option}-like construction to make it syntax driven.
...
\\\\
\rule[T-Let]{\Gamma \vdash t_1 : \sigma
\quad \Gamma[x \mapsto \sigma] \vdash t_2 : \tau}
{\Gamma\vdash \sletin{x = t_1}{t_2} : \tau}
\quad
\rule[T-Var]{}{\Gamma[x \mapsto \sigma]\vdash x : \sigma}
\\\\
\rule[T-Gen]{\Gamma \vdash t : \sigma}
{\Gamma \vdash t : \forall \alpha.\sigma}
(\mbox{if $\alpha$ not free in $\Gamma$})
\quad
\rule[T-Inst]{\Gamma \vdash t : \forall \alpha.\sigma}
{\Gamma \vdash t : \sigma[\tau / \alpha]}
\\\\
